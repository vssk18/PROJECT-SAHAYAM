...
def normalise_language(lang_raw: str) -> str:
...
def run_checklist_generator(name: str, phone_last4: str, lang_code: str,
...
def generate_report(participant_code: str,
    ...
    with out_file.open("w", encoding="utf-8") as r:
        ...
        if a["used_public_qr_for_payment"]:
            any_exposure = True
            r.write("- You used qr codes for payment without fully checking the name and amount\n")
        ...
        if a["mfa_after"]:
            r.write("- You have extra login protection on at least one important account\n")
        if a["screen_after"]:
        ...
        r.write("- Turn on extra login protection on email banking and main social accounts\n")
        ...
        r.write("- For bank issues use the phone number printed on your bank card or on the official bank website\n")
...
def main():
    ...
    print("2  extra login protection like two step verification")
    ...
    today_str = today = datetime.date.today().isoformat()
